
package resto;

import BaseDatos.MesaData;
import BaseDatos.PedidoData;
import BaseDatos.PedidoProductoData;
import BaseDatos.ProductoData;
import Entidades.Mesa;
import Entidades.Pedido;
import Entidades.PedidoProducto;
import Entidades.Producto;
import java.time.LocalDateTime;


public class Resto {

    public static void main(String[] args) {

//       ProductoData pd = new ProductoData();
//       Producto pro1 = new Producto(12,"empanadas salteñas picantes"
//               , 4000.000 ,5, true,"COMIDA");
//       Producto pro = new Producto(1);
//       System.out.println(pd.listarProductos()); //Funciona
//       pd.guardarProducto(pro);  //Funciona
//       pd.ModificarProducto(pro); //Funciona
//       pd.eliminarProducto(5);  //Funciona
//       MesaData md = new MesaData();
//       System.out.println(md.listarMesasId(1));       
//       System.out.println(md.listarMesas()); //Funciona
//       Mesa mesa= new Mesa(2,2,"LIBRE",6,true);
//       md.crearMesa(mesa);  //Funciona
//       md.eliminarMesa(4);  //Funciona
//       Mesa mesa1 = new Mesa(10, "LIBRE", 6);
//       Mesa mesa = new Mesa(1);
//       md.modificarMesa(mesa);  //Funciona
//       Pedido ped = new Pedido(15,mesa, "Carlos",
//            LocalDateTime.of(2023, 10, 10, 11, 00),
//            true, 0, "pendiente");
//       Pedido ped1 = new Pedido(2);
//       PedidoData ped = new PedidoData();
//       ped.guardarPedido(p1); //Funciona
//       pd.eliminarPedido(4);
//       System.out.println(pd.BuscarProductos("H")); //Funciona
//       System.out.println(pd.listarProdCateg(1)); //Funciona
//       System.out.println(ped.listarPedidos()); //Funciona
//       System.out.println(md.ObtenerMesasId(1)); //Funciona
//       System.out.println(pd.ObtenerPrductoId(1)); //Funciona
//       System.out.println(pro.toString());
//       PedidoProducto pedp = new PedidoProducto(ped1,pro,4,pro.getPrecio());
//       PedidoProductoData ppd = new PedidoProductoData();
//       ppd.crearPedProd(pedp);
//       System.out.println(ppd.BuscarProductosxIdPedido(2));
//       System.out.println(ped.listarPedidosMesaPendientes(1));
    }
}
